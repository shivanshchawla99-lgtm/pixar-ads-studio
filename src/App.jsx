      ...
      <aside className="shrink-0 bg-panel border-b lg:border-b-0 lg:border-r border-paper/8 flex flex-col lg:w-64">
        <div className="px-5 pt-5 pb-3 lg:px-6 lg:pt-7 lg:pb-6">
          <a href="#/" className="block">
            <div className="font-display font-bold text-2xl text-paper tracking-tight leading-none">
              Pixar <span className="text-lamplight">Ads</span> Studio
            </div>
            <div className="text-[11px] text-paper/50 mt-1">Unofficial concept demo</div>
            <div className="mt-2 inline-block rounded-full bg-lamplight/15 px-2.5 py-0.5 text-[11px] font-semibold text-lamplight">
              Story comes first
            </div>
        ...
        <nav className="flex gap-1 overflow-x-auto px-3 pb-3 lg:flex-1 lg:flex-col lg:overflow-visible lg:pb-0 lg:space-y-1 lg:gap-0">
          <a
            href="#/"
            className="relative flex shrink-0 items-center gap-2.5 whitespace-nowrap rounded-xl px-3 py-2.5 text-sm font-medium transition-[background-color,color,transform] duration-150 [transition-timing-function:var(--ease-out-strong)] active:scale-[0.98] text-paper/60 hover:text-lamplight hover:bg-lamplight/5"
          >
            ...
            return (
              <a
                ...
                className={`relative flex shrink-0 items-center gap-2.5 whitespace-nowrap rounded-xl px-3 py-2.5 text-sm font-medium transition-[background-color,color,transform] duration-150 [transition-timing-function:var(--ease-out-strong)] active:scale-[0.98] ${
                  active
                    ? 'text-lamplight font-semibold bg-lamplight/10'
                    : 'text-paper/60 hover:text-lamplight hover:bg-lamplight/5'
                }`}
              >
        ...
        <div className="hidden lg:block px-6 pb-5 text-[10px] leading-relaxed text-paper/35">
          Fan-made concept. Not affiliated with Pixar or Disney. All films fictional.
        </div>
      </aside>

      <div className="flex-1 min-w-0 flex flex-col">
        <header className="sticky top-0 z-30 bg-midnight/90 backdrop-blur border-b border-paper/8 px-4 sm:px-8 py-4">
          <div className="flex items-center gap-2.5">
            <span className={`h-3 w-3 rounded-full ${tool.dot}`} aria-hidden />
            <h1 className="text-2xl font-semibold font-display text-paper">{tool.name}</h1>
          </div>
          <p className="text-sm text-paper/60 mt-1">{tool.oneliner}</p>
        </header>
