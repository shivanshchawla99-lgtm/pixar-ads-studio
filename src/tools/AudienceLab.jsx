  ...
  return (
    <div className="space-y-6 fade-up relative overflow-hidden">
      ...
      <Card>
        ...
        {reviewCount > 0 && (
          <p className="text-xs text-paper/45 mt-3 inline-flex items-center gap-1">
      ...
      {!result ? (
        <Card>
          <div className="text-center py-6">
            <span className="material-symbols-outlined text-paper/25" style={{ fontSize: 36 }} aria-hidden>rate_review</span>
            <p className="text-sm text-paper/70 mt-3 max-w-md mx-auto">
        ...
        <div className="space-y-5 fade-up relative">
          <div className="rounded-2xl bg-panel-2 border border-sky/20 text-paper p-6 sm:p-7 relative overflow-hidden fade-up">
            ...
            <div className="relative z-10">
              ...
              <h2 className="font-display font-semibold text-2xl text-paper mt-1">What {film} audiences love</h2>
              <p className="text-paper/85 text-sm mt-2 max-w-2xl">{result.sentiment}</p>
              {result.thin && (
                <p className="rounded-lg bg-paper/10 px-3 py-2 text-xs inline-block mt-3">
          ...
          <div className="border-t border-paper/10">
            {result.loves.map((love, i) => (
              <div
                key={`${love.title}-${i}`}
                className="py-6 border-b border-paper/10 px-4 -mx-4 group transition-colors hover:bg-panel fade-up"
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="font-display font-semibold text-paper text-lg">{love.title}</h3>
                </div>
                <blockquote className={`mt-3 border-l-4 pl-3 italic text-sm text-paper/70 ${['border-sky', 'border-lamplight', 'border-teal', 'border-rose'][i % 4]}`}>
                  “{love.quote}”
                </blockquote>
                <div className="mt-4 grid md:grid-cols-2 gap-3">
                  {love.angles.map((a, j) => (
                    <div key={j} className={`rounded-xl p-4 ${['bg-sky/8', 'bg-lamplight/8', 'bg-teal/8', 'bg-rose/8'][i % 4]}`}>
                      <div className="font-display font-semibold text-sm text-lamplight">{a.hook}</div>
                      <div className="text-sm text-paper/75 mt-1.5">{a.primary_text}</div>
                      <div className="inline-flex items-start gap-1.5 text-xs text-paper/50 mt-2.5">
                      ...
                      <button
                        ...
                        className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-lamplight hover:underline cursor-pointer"
                      >
